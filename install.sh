...
on_install() {
  ...
  unzip -o "$ZIPFILE" README.md -d $TMPDIR >&2

  # update whitelist
  cp /sdcard/.sp_whitelist.txt $MODPATH/whitelist.txt 2>/dev/null \
    || mv $TMPDIR/whitelist.txt $MODPATH/
...
version_info() {
  ...
  ui_print " "
  cat $TMPDIR/README.md | while IFS= read -r line; do
    if $println; then
